...
class WooFunnels_DB_Updater {
	...
	public function __construct() {
		...
		add_action( 'before_delete_post', [ $this, 'bwf_update_delete_order' ] );
		add_action( 'wp_trash_post', [ $this, 'bwf_update_delete_order' ] );
		add_action( 'woocommerce_before_delete_order', [ $this, 'bwf_update_delete_order' ] );
		add_action( 'woocommerce_before_trash_order', [ $this, 'bwf_update_delete_order' ] );
	...
	public function woofunnels_wc_order_create_contact( $order_id, $posted_data, $order ) {
		...
		if ( $wp_id > 0 ) {
			...
			$email   = empty( $wp_user->user_email ) ? $email : $wp_user->user_email;
		}
	...
	public function schedule_order_reindex_action( $id = '' ) {
		if ( empty( $id ) || false === class_exists('WooCommerce') || ! function_exists( 'as_schedule_recurring_action' ) ) {
			return;
		}
	...
	/**
	 * Deleting order count and total spent
	 *
	 * @param $id
	 *
	 * @return void
	 */
	public function bwf_update_delete_order( $id ) {
		if ( did_action( 'woocommerce_before_delete_order' ) || did_action( 'woocommerce_before_trash_order' ) ) {
			return;
		}
		...
		$this->schedule_order_reindex_action( $id );
	}
	...
	public function bwf_reindex_contact_orders( $cid ) {
		...
		$orders_ids = $wpdb->get_col( $wpdb->prepare( $sql, array( '_billing_email', $bwf_contact->get_email(), 'shop_order', $indexed_order_id ) ) );
		if ( empty( $orders_ids ) ) {
			$this->un_schedule_wc_recurring_actions( $cid );
