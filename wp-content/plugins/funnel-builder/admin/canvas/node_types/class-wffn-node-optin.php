	...
	public function register_a_node( &$index, &$configs, &$edge_index ) {
		...
		$get_parent_config['tabs']  = array(
			array(
				...
				'api'   => defined('WFFN_PRO_FILE') ? 'woofunnels-admin/canvas/' . $this->data['funnel_id'] . '/nodes/analytics/' . $this->id . '?type=optin' . $view_experiment : '',
			),
			array(
				'name'  => 'contacts',
				'title' => __( 'Contacts', 'funnel-builder' ),
				'api'   => defined('WFFN_PRO_FILE') ? 'woofunnels-admin/canvas/' . $this->data['funnel_id'] . '/nodes/contacts/' . $this->id . '?type=optin' . $view_experiment : '',
			),
			array(
				'name'  => 'experiment',
