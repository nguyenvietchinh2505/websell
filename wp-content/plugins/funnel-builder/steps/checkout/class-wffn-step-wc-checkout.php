...
if ( ! class_exists( 'WFFN_Step_WC_Checkout' ) ) {
	class WFFN_Step_WC_Checkout extends WFFN_Step {
		...
		public function get_entity_tags( $step_id, $funnel_id ) {
			...
			$flags          = [];
			if ( count( $wfacp_products ) < 1 && absint( $funnel_id ) !== WFFN_Common::get_store_checkout_id() ) {
		...
		public function alter_templates( $templates ) {
			...
			$templates['wc_checkout']['customizer'] = $templates['wc_checkout']['pre_built'];
			$templates['wc_checkout']['wp_editor']  = $templates['wc_checkout']['embed_forms'];
