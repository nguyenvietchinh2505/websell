...
class WFACP_Reporting {
	...
	public function create_table() {
		...
		dbDelta( $creationSQL );
		update_option( 'wfacp_db_ver_1_0', gmdate( 'Y-m-d' ) );
	}
	...
	public function insert_row_for_ipn_based_gateways( $order_id, $from, $to ) {
		...
		/**
		 * condition1 : check if current method is not IPN based
		 * condition2: already ran thankyou hook with not paid status (including IPN and non IPN gateways)
		 */
	...
	public function update_reporting_data_in_meta( $order, $posted_data ) {
		...
		if ( isset( $posted_data['wfacp_woocommerce_pay'] ) && true === $posted_data['wfacp_woocommerce_pay'] ) {
			update_post_meta( $order->get_id(), '_wfacp_report_data', array( 'wfacp_total' => $wfacp_used_total, 'funnel_id' => $funnel_id ) );
		} else {
	...
	public function updating_reports_from_orders( $order_id ) {
		...
		delete_post_meta( $order_id, '_wfacp_report_data' );
		delete_post_meta( $order_id, '_wfacp_report_needs_normalization' );
