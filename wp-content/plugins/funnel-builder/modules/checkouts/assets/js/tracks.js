...
(function ($) {
    ...
    class Events {
        ...
        attach_triggers() {
            ...
            $(document.body).on('wfacp_product_added wfob_product_added', function (e, v) {

                if (v.hasOwnProperty('item_key') && v.hasOwnProperty('item') && v.item.hasOwnProperty(v.item_key)) {
                    let item_key = v.item_key;
                    if (undefined == self.data.add_to_cart) {
                        return;
                    }
                    if (self.data.add_to_cart.hasOwnProperty(item_key)) {
                        return;
                    }

                    if (v.item[item_key].hasOwnProperty(self.track_name)) {
                        self.data.add_to_cart[item_key] = v.item[item_key];
                        self.single_add_to_cart(v.item[item_key][self.track_name], v.item.cart_key);
                    }
        ...
        add_item(v, by_pass = false) {
            if (v.hasOwnProperty('item_key') && v.hasOwnProperty('item') && v.item.hasOwnProperty(v.item_key)) {
                ...
                if (v.item[item_key].hasOwnProperty(this.track_name)) {
                    ...
                    this.single_add_to_cart(v.item[item_key][this.track_name], v.item.cart_key, by_pass);
                }
                this.checkout_event_run = false;
            }
        }
    ...
    class Facebook extends Events {
        ...
        event_payment_info() {
            if (typeof fbq === 'function') {
                this.fbq('AddPaymentInfo', this.data.checkout);
            }
        }
    ...
    class TikTok extends Events {
        ...
        ttq(event, data) {
            ...
            let event_id = this.get_event_id();
            ttq.instance(this.track_id).track(event, data);
        }
        ...
        event_checkout(checkout_data) {
            let c_data = JSON.parse(checkout_data);
            this.ttq('InitiateCheckout', c_data);
        }
