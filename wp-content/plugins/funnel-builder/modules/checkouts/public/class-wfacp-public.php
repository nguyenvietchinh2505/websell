...
class WFACP_Public {
	...
	public function calculate_totals( $ins ) {
		...
		if ( ! wp_doing_ajax() || WFACP_Common::get_id() == 0 || apply_filters( 'wfacp_disabled_discounting', false, $this ) ) {
			return $ins;
		}
	...
	*/
	...
	public function shimmer_css() {
		...
		if ( apply_filters( 'wfacp_shimmer_active', true ) ) {
			ob_start();
			echo "<style>";
			include_once plugin_dir_path( WFACP_PLUGIN_FILE ) . 'assets/css/wfacp-shimmer.min.css';
			echo "</style>";
			$shimmer_css = ob_get_clean();
			echo $shimmer_css;
		} else {
